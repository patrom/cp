...
//		assertEquals(24, noTie.getDisplayLength());
...
//	}
	...
	public void testTiesHalfNote2(){
		List<Note> notes = new ArrayList<>();
		notes.add(note().pc(0).pitch(60).len(DurationConstants.HALF).pos(0).build());
		notes.add(note().pc(2).pitch(62).len(DurationConstants.EIGHT).pos(DurationConstants.HALF).build());
		notes.add(note().pc(3).pitch(63).len(DurationConstants.QUARTER).pos(DurationConstants.EIGHT).build());
		beatMap.createBeatMap(notes, DurationConstants.QUARTER);
		List<Note> ties = beatMap.createTies();
		assertEquals(4, ties.size());
		assertEquals(DurationConstants.QUARTER, ties.get(0).getDisplayLength());
		assertEquals(DurationConstants.QUARTER, ties.get(1).getDisplayLength());
		assertEquals(DurationConstants.EIGHT, ties.get(2).getDisplayLength());
	}
