...
//        LOGGER.info("melodic harmonic " + avg);
    ...
    protected double getMelodyDissonance(List<Note> notes, int voice) {
        ...
        List<Chord> chords = extractChords(notes, melodyDissonance.getChordSize());
        if (chords.isEmpty()) {
            return 0;
        } else {
    ...
    protected List<Chord> extractChords(List<Note> notes, int size) {
        Stream<List<Note>> slidingWindow = sliding(notes, size);
        return  slidingWindow.map(windowNotes -> {
                                                    return new Chord(windowNotes);
